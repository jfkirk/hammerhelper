...
def roll_from_dice_string(dice_string, size):
    if dice_string == 'D3':
        ...
        return np.random.randint(1, 6, size=size)
    else:
...
def simulate_unit(attacking_unit, weapon, n_shots, target_unit, offensive_abilities, friendly_army, enemy_army):
    ...
    relevant_unit_skill = attacking_unit.ws if weapon.weapon_type == WeaponType.MELEE else attacking_unit.bs
    ...
    for sim in range(N_SIMS):
        ...
        if 'D' in str(weapon.effective_attacks):  # TODO don't do this string conversion in the tight loop
            n_attacks = sum(roll_from_dice_string(weapon.effective_attacks, size=n_shots))
        else:
            n_attacks = n_shots

        # Roll the attack dice
        hit_rolls = np.random.randint(1, 6, size=n_attacks)
        for ability in all_abilities:
            hit_rolls = ability.modify_hit_rolls(hit_rolls)
        n_hits = (hit_rolls >= relevant_unit_skill).sum()

        # Roll the wound dice
        wound_rolls = np.random.randint(1, 6, size=n_hits)
        n_wounds = (wound_rolls >= wound_target_value).sum()

        # Roll the save dice
        save_rolls = np.random.randint(1, 6, size=n_wounds)
    ...
    all_attacks.sort()
    attacks_result = (all_attacks[index_25th], all_attacks[index_median], all_attacks[index_75th])

    all_hits.sort()
    hits_result = (all_hits[index_25th], all_hits[index_median], all_hits[index_75th])

    all_wounds.sort()
    wounds_result = (all_wounds[index_25th], all_wounds[index_median], all_wounds[index_75th])

    all_damages.sort()
    damages_result = (all_damages[index_25th], all_damages[index_median], all_damages[index_75th])

    all_kills.sort()
    kills_result = (all_kills[index_25th], all_kills[index_median], all_kills[index_75th])

    return [attacks_result, hits_result, wounds_result, damages_result, kills_result]
